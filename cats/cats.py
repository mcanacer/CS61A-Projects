...
def about(topic):
    ...
    def goal(string):
        result = remove_punctuation(string).lower().split()
        index = 0
        finish = len(topic)
        while finish > index:
            if topic[index] in result:
                return True
            index += 1
        return False
    return goal
...
def accuracy(typed, reference):
    ...
    if len(typed_words) > len(reference_words):
        ...
        for i in range(len(typed_words)):
            if typed_words[i] == reference_words[i]:
                total += 1
    return total * 100 / len(typed_words)
...
def time_per_word(words, times_per_player):
    ...
    for i in range(len(times_per_player)):
        ...
        for a in range(len(words)):
            total[i].append(times_per_player[i][a + 1] - times_per_player[i][a])
    return {"words": words, "times": total}
